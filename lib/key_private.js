...
PrivateKey.randomKey = function () {
    ...
    var init = PrivateKey.initialize();
    var getRandom32ByteBuffer = keyUtils.random32ByteBuffer({ cpuEntropyBits: cpuEntropyBits });
...
PrivateKey.unsafeRandomKey = function () {
    return keyUtils.random32ByteBuffer({ safe: false }).then(function (value) {
        return PrivateKey.fromBuffer(value);
    });
};
